...
import torch
import pygame 
from zbot_env_vmp import ZbotEnvVMP  # Use your VMP-based environment
...
def keyboard_control_policy(obs: torch.Tensor) -> torch.Tensor:
    ...
    # Apply user commands to observation
    obs[:, 6] = USER_CMD["x"]
    obs[:, 7] = USER_CMD["y"]
    obs[:, 8] = USER_CMD["yaw"]
    print('obs shape real before: ', obs.shape)
    
    # Include VMP latent representations in obs
    if hasattr(env, "vae") and env.vae is not None:
        latent = env.current_z.view(1, -1).expand(obs.shape[0], -1)
        motion = env.current_m.view(1, -1).expand(obs.shape[0], -1)
        obs = torch.cat([obs, latent, motion], dim=-1)

    # ✅ Return only the first 123 dimensions to match the trained policy
    obs = obs[:, :123]

    print(f'Obs shape after: {obs.shape}')
    with torch.no_grad():
        actions = policy.act_inference(obs)  # Use deterministic inference mode
    ...
    try:
        while rollouts_completed < num_rollouts:
            if use_keyboard:
                ...
                with torch.no_grad():
                    modified_obs = modified_obs[:, :123]
                    print(f"Observation shape: {obs.shape}, modified_obs: {modified_obs.shape}")
                    actions = policy_fn.act_inference(modified_obs)
                    print('this done')
            ...
            if dones.item():
                ...
                if rollouts_completed < num_rollouts:
                    obs = env.reset()  # Ensures that VMP latents and motion states are initialized
...
def main():
    ...
    parser.add_argument("-e", "--exp_name", type=str, default="run_new_zbot_2")
    ...
    env = ZbotEnvVMP(
        ...
        vae_model_path="genesis_playground/zbot/models/ws15_ld64_beta0.05_run5/model.pth",  # Ensure your trained VAE model is loaded
    ...
    # Instantiate your trained policy
    policy = VAEConditionedActorCritic(
        num_obs=env.num_obs,
        #num_obs = 207,
        num_privileged_obs=env.num_obs,  # Since we're using the same obs for actor/critic
        #num_privileged_obs = 207,
        num_actions=env.num_actions,
        actor_hidden_dims=[512, 256, 128],  
        critic_hidden_dims=[512, 256, 128],  
        activation="elu",
        init_noise_std=0.1,  # Match what you trained with
        noise_std_type="log",
        conditional_dim=64 + 20  # Ensure this matches your VMP conditioning setup
    ).to(args.device)
